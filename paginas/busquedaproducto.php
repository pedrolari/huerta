<?php
	...
	$subcatpro = $_POST['subcategoria'];
	$ofertaspro = $_POST['ofertaspro'];
	$novedadespro = $_POST['novedadespro'];
	...
	if($preciopro == 0) {
		if($consultaavanzada != '') {
		$consultaavanzada .= " AND producto.precio < 4";
		} else {
		$consultaavanzada = "producto.precio < 4";
		}
	}
	if($preciopro == 1) {
		if($consultaavanzada != '') {
	...
	if($subcatpro != '') {
		if($consultaavanzada != '') {
			$consultaavanzada .= " AND producto.idcat LIKE '$subcatpro'";
		} else {
			$consultaavanzada = "producto.idcat LIKE '$subcatpro'";
		}
	}
	...
	if($novedadespro == 1) {
		...
		if($consultaavanzada != '') {
		$consultaavanzada .= " AND DATEDIFF( CURDATE(), productos_productor.fechaalta ) < 15";
		} else {
		$consultaavanzada = "DATEDIFF( CURDATE(), productos_productor.fechaalta ) < 15";
		}
	}
	if($novedadespro != 1) {
		if($consultaavanzada != '') {
	...
	if($ofertaspro == 1) {
		if($consultaavanzada != '') {
			$consultaavanzada .= " AND producto.oferta = 1";
		} else {
			$consultaavanzada = "producto.oferta = 1";
		}
	}
	if($ofertaspro != 1) {
		if($consultaavanzada != '') {
			$consultaavanzada .= " AND producto.oferta = 0";
		} else {
			$consultaavanzada = "producto.oferta = 0";
		}
	}
	
	//SELECT producto.id_producto as id, producto.nombre as nombre, DATEDIFF( CURDATE(), productos_productor.fechaalta ) AS dias FROM categoria, producto, productos_productor WHERE producto.id_producto=productos_productor.id_producto AND producto.idcat=categoria.idcat
